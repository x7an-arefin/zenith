...
export function parseGiteeError(status: number, data: { error?: { message?: string }; message?: string }): Error {
  ...
  if (status === 401 || lower.includes('unauthorized') || lower.includes('invalid api key')) {
    return Object.assign(new Error(`Gitee AI auth failed: ${msg}`), { _zenithErrorCategory: 'auth' as const, _zenithStatus: status })
  }
  if (status === 429 || lower.includes('rate limit') || lower.includes('too many')) {
    return Object.assign(new Error(`Gitee AI rate limited: ${msg}`), { _zenithErrorCategory: 'rate-limit' as const, _zenithStatus: status })
  }
  if (lower.includes('quota') || lower.includes('exceeded') || lower.includes('insufficient')) {
    return Object.assign(new Error(`Gitee AI quota exceeded: ${msg}`), { _zenithErrorCategory: 'quota' as const, _zenithStatus: status })
  }
  if (msg.toLowerCase().includes('expired')) {
    return Object.assign(new Error(`Gitee AI token expired: ${msg}`), { _zenithErrorCategory: 'auth' as const, _zenithStatus: status })
  }
  return Object.assign(new Error(`Gitee AI error (${status}): ${msg}`), { _zenithErrorCategory: 'provider' as const, _zenithStatus: status })
}

export async function generateGiteeImage(request: ImageRequest, token: string): Promise<ImageResult> {
